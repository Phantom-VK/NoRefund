...
_PROCESSING_GRAB_RETRY_MS = 30
_PROCESSING_MAX_GRAB_ATTEMPTS = 10


class ProcessingModal(ctk.CTkToplevel):
    """Blocks the parent window while a tokenization job runs in the
    ...
    def __init__(self, parent, title: str, on_cancel: Callable[[], None]) -> None:
        ...
        self._status_label.pack(pady=(theme.SPACE_3, 0))

        self._cancel_btn = IconButton(
            body, "Cancel", icon="x", variant="danger", command=self._cancel
        )
        self._cancel_btn.pack(pady=(theme.SPACE_4, 0))
