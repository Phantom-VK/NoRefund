...
badge's text-on-tint. Uses the same tint()/blend math the app itself uses
to build these colors, so a color-token change that breaks contrast again
fails a test instead of only being caught by eye."""

from __future__ import annotations

from norefund.gui.formatting import _hex_to_rgb, tint
...
def test_provider_badges_clear_aa_in_both_modes():
    # Mirrors ProviderBadge's own construction: a light-alpha tint for the
    ...
    for provider, accent in PROVIDER_COLORS.items():
        for mode in (0, 1):
            bg = tint(accent, card[mode], bg_alpha[mode])
            text = tint("#000000" if mode == 0 else "#ffffff", accent, text_blend)
